...
class Layered_indexable_feature_lang_value extends \Dingedi\TablesTranslation\DgTableTranslatable17
{
    public $module = 'blocklayered';
    public $table = 'layered_indexable_feature_lang_value';

    public function __construct()
    {
        $primary_key = 'id_feature';
    ...
    public function beforeAction($objectSource, $objectDest, $class)
    {
        if ($objectSource['meta_title'] !== "") {
            ...
            $item = \Db::getInstance()->executeS("SELECT * FROM " . _DB_PREFIX_ . "feature_lang WHERE id_feature = " . $objectDest['id_feature'] . " AND id_lang = " . $objectDest['id_lang']);
            $item = $item[0];
