...
class Layered_indexable_feature_value_lang_value extends \Dingedi\TablesTranslation\DgTableTranslatable17
{
    public $module = 'blocklayered';
    public $table = 'layered_indexable_feature_value_lang_value';
    ...
    public function beforeAction($objectSource, $objectDest, $class)
    {
        if ($objectSource['meta_title'] !== "") {
            ...
            $item = \Db::getInstance()->executeS("SELECT * FROM " . _DB_PREFIX_ . "feature_value_lang WHERE id_feature_value = " . $objectDest['id_feature_value'] . " AND id_lang = " . $objectDest['id_lang']);
            $item = $item[0];
